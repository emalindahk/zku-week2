...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
      ...
      console.log(recipientBalance.toString(), omniBridgeBalance.toString())
  })
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    ...
    // Alice withdraws all of her funds in L1
    const aliceWithdrawAmount = utils.parseEther('0.07');
    const alicerecipient = '0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2'
    ...
    const aliceBalanceAfterWithdraw = await token.balanceOf(aliceAddress)
    expect(aliceBalanceAfterWithdraw).to.be.equal(0)
    const bobBalanceAfterWithdraw = await token.balanceOf(bobAddress)
    expect(bobBalanceAfterWithdraw).to.be.equal(bobWithdrawAmount)
    const omniBridgeBalance = await token.balanceOf(omniBridge.address)
    expect(omniBridgeBalance).to.be.equal(aliceWithdrawAmount)
  })
})
